---
name: build-ai-voice
description: "WaitZero AI/음성 기능 구현. Flutter speech_to_text 음성 인식, Spring Boot OpenAI API (Spring AI) 연동, 민원 자동 분류/요약 기능을 구현한다. '음성 인식', 'AI 분류', 'OpenAI API (Spring AI)', '음성 입력' 요청 시 사용."
---

# WaitZero AI/음성 기능 구현

음성 인식(Flutter) + AI 민원 분류(Spring Boot + OpenAI API (Spring AI)) 기능을 구현한다.

## 구현 순서

### 1. 백엔드 — OpenAI API (Spring AI) 연동

`com.waitzero.ai` 패키지:

**AiClassifyService:**
- OpenAI API (Spring AI)로 민원 내용 → 카테고리 자동 분류
- 시스템 프롬프트:
  ```
  ...
  ```
- Spring AI `ChatClient`로 OpenAI API 호출
- API 키: `${OPENAI_API_KEY}` (application.yml → `spring.ai.openai.api-key`)
...
- OpenAI API (Spring AI) 실패: "자동 분류를 사용할 수 없습니다. 카테고리를 직접 선택해주세요."
