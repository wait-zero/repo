...
public interface PreRegistrationRepository extends JpaRepository<PreRegistration, Long> {
    ...
    @Query("SELECT r FROM PreRegistration r JOIN FETCH r.office WHERE r.user.id = :userId ORDER BY r.createdAt DESC")
    List<PreRegistration> findByUserIdWithDetails(@Param("userId") Long userId);

    @Query("SELECT r FROM PreRegistration r JOIN FETCH r.office JOIN FETCH r.user WHERE r.id = :id")
